...
impl EdgeStack {
    ...
    pub fn get_item_in_stack_pos_as_u16(&self, pos: u16) -> u16 {
        ...
        assert!(count < TRACKED_ITEM_COUNT, "tracked item count is too large for stack_items bit vec");
    ...
    pub fn stack_update_with_bit_ops(
        mut self,
        stack_entry_pos_to_stack_exit_pos: &HashMap<u16, HashSet<u16, FnvBuildHasher>, FnvBuildHasher>,
        ...
        for (stack_entry_pos, stack_exit_pos_set) in stack_entry_pos_to_stack_exit_pos.iter() {
            ...
            if stack_pos & bit != 0 {
                ...
                stack_pos &= !bit;
                let pos_index = initially_set_bits.iter().position(|&x| x == *stack_entry_pos).expect("initially_set_bits does not contain stack_entry_pos");
                ...
                for stack_exit_pos in stack_exit_pos_set.iter() {
                    ...
                    if stack_exit_pos <= &127 { // cant go over max stack size
        ...
        if exit_pos_vec.len() > TRACKED_ITEM_COUNT {
            println!("exit_pos_vec.len() > TRACKED_ITEM_COUNT: {} > {}", exit_pos_vec.len(), TRACKED_ITEM_COUNT);
        ...
        for (i, (stack_item, stack_exit_pos)) in exit_pos_vec.into_iter().take(TRACKED_ITEM_COUNT).enumerate() {
...
pub fn symbolic_cycle(
    ...
    let mut visited_set: HashSet<EdgeSet, FnvBuildHasher> = HashSet::with_capacity_and_hasher(2056, FnvBuildHasher::default());
    let mut unvisited_queue: VecDeque<(&InstructionBlock, EdgeStack)> = VecDeque::with_capacity(1024);
    ...
    while !unvisited_queue.is_empty() {
        ...
        let new_stack_size = (current_stack.stack_size as i16) + current_block.stack_info.stack_size_delta;
        if new_stack_size < 0 {
        ...
        if *last_op == 0x56 {
            ...
            let jump_usage = current_block.stack_info.get_entry_stack_usage_by_pc(*last_pc);
            if jump_usage.is_empty() {
                ...
                if let Some(_push_val_for_jump) = &current_block.stack_info.push_used_for_jump {
                    ...
                    let new_stack = current_stack.stack_update_with_bit_ops(&current_block.stack_info.stack_entry_pos_to_stack_exit_pos, current_block.stack_info.stack_size_delta, &current_block.push_vals, set_all_valid_jumpdests);
                    ...
                    println!("Symbolic jump to location created within block at jump pc: {last_pc}");
                    if label_symbolic_jumps {
                        ...
                        for pc in set_all_valid_jumpdests {
                            ...
                            let jumpdest_block = cfg_runner.map_to_instructionblock.get(&next_node).unwrap();
                            if current_stack.stack_size > jumpdest_block.stack_info.min_stack_size_required_for_entry {
                                cfg_runner.cfg_dag.add_edge(current_node, next_node, Edges::SymbolicJump);
                        ...
                        for next_node in next_nodes {
                            ...
                            // add to visited_set and push new state to queue
                            visited_set.insert(new_edge_set);
                            let _next_block = cfg_runner.map_to_instructionblock.get(&next_node).unwrap();
                ...
                if current_stack.is_item_in_pos_set(entry_pos) {
                    ...
                    // add edge to cfg
                    cfg_runner.cfg_dag.add_edge(current_node, next_node, Edges::Jump);
